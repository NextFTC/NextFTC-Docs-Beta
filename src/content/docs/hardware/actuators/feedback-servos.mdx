...
The servo port must be in the range `[0, 5]`.
Both constructors also accept the same optional `cacheTolerance` parameter as their base class.

`feedback` is an `AnalogFeedback` instance, constructed by hardware map name as shown above.
You can also pass any custom `AnalogFeedback` implementation, or construct a `NextAnalogInput` directly by Lynx Module and port:
